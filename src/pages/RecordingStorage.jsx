...
function RecordingStorage() {
    ...
    const handleAudioClick = async (audio) => {
        ...
        if (activeAudio?.s3_key === audio.s3_key) {
            ...
            try {
                ...
                const response = await fetch(`https://54.179.196.155:8000/transcribe/get_transcription/${audio.s3_key}`, {
                    method: "GET",})
    ...
    const handleDelete = async (s3_key) => {
        try {
            const response = await fetch(`https://54.179.196.155:8000/s3/audio/delete/${s3_key}`, {
                method: "DELETE",
            });
            if (response.ok) {
    ...
    const handleEdit = async (s3_key, newTitle) => {
        try {
            ...
            const response = await fetch(`https://54.179.196.155:8000/s3/audio/edit/${s3_key}`, {
                method: "PUT",
                headers: {
